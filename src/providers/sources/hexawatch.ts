...
// 🔐 X-AUTH generator
function generateXAuth(type: "movie" | "tv", tmdbData: TMDBMovieData | TMDBEpisodeData): string {
  ...
  const encryptedLevel1 = CryptoJS.AES.encrypt(JSON.stringify(payloadLevel1), Gl).toString();
  const encryptedLevel2 = CryptoJS.AES.encrypt(JSON.stringify({ data: encryptedLevel1, timestamp }), `${Gl}${timestamp}`).toString();
  const encryptedLevel3 = CryptoJS.AES.encrypt(JSON.stringify({ data: encryptedLevel2, timestamp, random }), Kl).toString();
...
function decryptPayload(encryptedStr: string): DecryptedPayload | null {
  ...
  const level2 = decryptAES<PayloadLevel2>(level1.data, Gl + level1.timestamp, "Level 2");
...
// 🔍 Best stream finder
async function findBestStream(sources: StreamSource[]): Promise<StreamSource | null> {
